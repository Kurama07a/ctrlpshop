...
async function fetchShopInfo(userEmail) {
  try {
    console.log("fetchShopInfo called for email:", userEmail);
    
    if (!userEmail) {
      console.error("No email provided to fetchShopInfo");
      return { success: false, error: "No email provided" };
    }
    
    const supabase = getSupabaseClient();
    console.log("Querying shop_accounts for email:", userEmail);
    ...
    shopInfo = {
      ...
      owner_name: data.owner_name || "Not Proided",
    ...
    if (mainWindow) {
      console.log("Sending shop-info-fetched event with data:", shopInfo);
