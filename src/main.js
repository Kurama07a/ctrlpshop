...
function createMainWindow() {
  mainWindow = new BrowserWindow({
    ...
    console.log('Loading from:', indexPath); // <-- Try this or adjust as needed  // For production
    if (fs.existsSync(indexPath)) {
      mainWindow.loadFile(indexPath);
    } else {
      console.error('Build files not found. Run "npm run build" first.');
      // Fallback to a simple error page
      mainWindow.loadURL(`data:text/html,<h1>Build files not found. Run "npm run build" first.</h1>`);
    }
  }
